...
- **`from __future__ import annotations`**: every module starts with it. On Python 3.14 (PEP 649) annotations are lazily evaluated against the *defining scope*, so a method named like a builtin (e.g. `inbox.list`) shadows that builtin in a sibling method's annotation (`list[Message]`) and crashes `get_type_hints`. The future-import keeps annotations as strings resolved against module globals, avoiding this footgun.
- **Style**: ruff (line-length 100; E/F/UP/B/I), basedpyright-clean (0 errors/0 warnings), codespell-clean. 4-space indent. basedpyright runs in its strict default for `src`; `tests`/`devtools` get relaxed rules via `executionEnvironments` (loosely-typed fixtures/mocks, private-attr introspection) while keeping semantic checks like `reportArgumentType`/`reportCallIssue`/`reportOptionalMemberAccess` on.

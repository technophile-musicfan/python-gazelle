...
The transport layer handles all HTTP communication with Gazelle-based tracker APIs. It is responsible for authentication (cookie-based and API key), rate limiting via a token-bucket algorithm, retry logic with exponential backoff for transient errors, and surfacing a typed exception hierarchy so callers can handle error conditions precisely.

# Capability: cross-upload

## Purpose

Help a user upload a release they have on one Gazelle tracker to a second tracker with correct metadata, structured as two phases so the consequential live write is isolated and opt-in. A read-only preparation step maps the source release's metadata onto the target tracker's upload schema (best-effort, flagging anything it cannot confidently map for review), detects duplicates on the target, and returns a draft — performing no write. A separate explicit submission step validates the draft, refuses to re-upload an existing release unless overridden, and POSTs the upload. The caller builds the target `.torrent` (the library exposes the target's announce URL and treats the supplied `.torrent` as opaque bytes — it never parses or generates one). Available on async and synchronous surfaces.
